...
void VignetteEffect::Initialize(ID3D12Device* device) {
	assert(device != nullptr);

	InitializePipeline(device);
}

void VignetteEffect::Draw(ID3D12GraphicsCommandList* commandList, uint32_t srvIndex) {
...
void VignetteEffect::InitializePipeline(ID3D12Device* device) {
    ...
    hr = vignetteConstBuffer_->Map(0, nullptr, reinterpret_cast<void**>(&mappedData_));
    assert(SUCCEEDED(hr));
    *mappedData_ = settings_;
