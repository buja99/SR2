...
void RadialBlurEffect::Initialize(ID3D12Device* device) {
...
void RadialBlurEffect::Draw(ID3D12GraphicsCommandList* commandList, uint32_t srvIndex) {
...
void RadialBlurEffect::InitializePipeline(ID3D12Device* device) {
    HRESULT hr;
    ...
    hr = device->CreateCommittedResource(
        ...
        D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&radialBlurConstBuffer_));
    assert(SUCCEEDED(hr));

    hr = radialBlurConstBuffer_->Map(0, nullptr, reinterpret_cast<void**>(&mappedData_));
    assert(SUCCEEDED(hr));
